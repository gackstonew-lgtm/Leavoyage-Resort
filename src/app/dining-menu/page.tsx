...
import { DINING_CATEGORIES } from '@/data/menuData';
import MenuItemCard from '@/components/menu/MenuItemCard';
import MenuNavigationTabs from '@/components/menu/MenuNavigationTabs';
import MenuCategoryNav from '@/components/menu/MenuCategoryNav';
import { Search, Utensils, MessageCircle, Sparkles } from 'lucide-react';
...
export default function DiningMenuPage() {
  ...
  const filteredCategories = useMemo(() => {
    return DINING_CATEGORIES.map((category) => {
  ...
  return (
    <div className="pt-28 pb-28">
      ...
      <div className="bg-resort-950 text-white py-16 mb-8 border-b border-resort-800 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-3">
          <span className="text-xs font-bold uppercase tracking-widest text-resort-400 block">
            Culinary Craft & Fresh Flavors
          </span>
          <h1 className="text-3xl sm:text-5xl font-serif font-bold text-white">
            LE-VOYAGE Resort Dining Menu
          </h1>
          <p className="text-slate-300 text-xs sm:text-sm max-w-2xl mx-auto leading-relaxed">
            Savor authentic African fusion, prime grilled platters, fresh lake tilapia, and delectable hot & cold beverages prepared fresh daily in Kitale.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <MenuNavigationTabs activeMenu="dining" />
        ...
        <div className="relative max-w-md mx-auto">
          ...
          <input
            type="text"
            placeholder="Search food items (e.g. Tilapia, Chicken, Steak, Juice)..."
            value={searchQuery}
        ...
        <div className="space-y-12">
          {filteredCategories.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 p-8 space-y-3">
              ...
              <h3 className="text-base font-serif font-bold text-slate-800">No menu items found</h3>
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                We couldn&apos;t find any dining items matching &quot;{searchQuery}&quot;. Try adjusting your search query or category filter.
              </p>
              <button
                onClick={() => {
        ...
        {/* Private Dining & Table Reservation Banner */}
        <div className="bg-gradient-to-r from-resort-950 via-resort-900 to-resort-950 text-white rounded-2xl p-8 sm:p-10 shadow-xl border border-resort-800 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-resort-800/80 border border-resort-700 text-resort-300 text-xs font-semibold">
            ...
            <span>Table Reservations & Catering</span>
          </div>
          <h3 className="text-2xl sm:text-3xl font-serif font-bold">
            Hosting a Special Dinner or Outdoor Event?
          </h3>
          <p className="text-slate-300 text-xs sm:text-sm max-w-xl mx-auto leading-relaxed">
            Our master culinary team provides bespoke buffet menus, private garden shade barbecues, and cocktail setups tailored for your group.
          </p>
          <div className="pt-2">
            <a
              href={getWhatsAppLink('Hello LE-VOYAGE Resort, I would like to enquire about table reservations and custom food orders.')}
              target="_blank"
            ...
            >
              ...
              <span>Contact Food & Beverage Desk</span>
            </a>
          </div>
        </div>
